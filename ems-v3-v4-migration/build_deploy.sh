...
sam deploy --no-execute-changeset --no-fail-on-empty-changeset --capabilities CAPABILITY_IAM --template-file template.yaml --stack-name my-stack-name-$env --parameter-overrides Environment=$env --s3-bucket sam-temp-test-ems --capabilities CAPABILITY_IAM
